"""
Sports Celebrity Image Classification: Refactored Data Cleaning
...
2. Uses flexible face detection strategies (no strict eye requirement)
...
Updated to use centralized configuration from config module.
"""
...
class ImageDataCleaner:
    ...
    def __init__(
        ...
        Args:
            ...
            cropped_path: Path where cropped images will be saved
        """
        ...
        if self.face_cascade.empty():
            raise RuntimeError(f"Failed to load face cascade from: {config.FACE_CASCADE_PATH}")
        if self.eye_cascade.empty():
            raise RuntimeError(f"Failed to load eye cascade from: {config.EYE_CASCADE_PATH}")

    def get_cropped_image_if_face_detected(self, image_path: str) -> Optional[np.ndarray]:
        """
        ...
        Returns:
            Cropped face image if valid, None otherwise
        """
        ...
        if len(faces) > 0:
            ...
            # Basic quality check using config
            if (roi_color.shape[0] >= config.MIN_FACE_SIZE[1] and
                roi_color.shape[1] >= config.MIN_FACE_SIZE[0]):
                return roi_color

        return None

    def get_cropped_image_if_2_eyes(self, image_path: str) -> Optional[np.ndarray]:
        """
    ...
    def get_cropped_image_with_eye_validation(
        self,
        image_path: str
    ) -> Optional[np.ndarray]:
        """
        ...
        for (x, y, w, h) in faces:
            ...
            elif len(eyes) >= config.MIN_EYES_MEDIUM_QUALITY:
                return roi_color
    ...
    def visualize_face_detection(self, image_path: str) -> None:
        """
    ...
    def get_image_directories(self) -> List[str]:
        """
        Get all subdirectories in the dataset path, excluding the cropped folder.

        Returns:
            List of directory paths containing celebrity images
        """
        img_dirs = []
    ...
    def setup_cropped_directory(self) -> None:
    ...
    def process_all_images(
        ...
        Returns:
            Tuple of (list of cropped directories, dict of celebrity file names)
        """
        ...
        for img_dir in img_dirs:
            ...
            for entry in os.scandir(img_dir):
                if entry.is_file() and entry.name.lower().endswith(tuple(config.SUPPORTED_FORMATS)):
    ...
    def get_processing_stats(self, celebrity_file_names_dict: Dict[str, List[str]]) -> None:
        """
        Print statistics about the processed dataset.

        Args:
            celebrity_file_names_dict: Dictionary mapping celebrities to their processed files
        """
        print("\n" + "="*50)
...
def main():
    ...
    print("Starting Sports Celebrity Image Classification - Refactored Data Cleaning")
